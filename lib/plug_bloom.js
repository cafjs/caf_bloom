...
var calculateIndexes = function(key, size, slices) {
    var hash1 = fnv.hash('one:' + key, 32).value;
    var hash2 = fnv.hash('two:' + key, 32).value;
    var result = [];
...
exports.newInstance = function($, spec, cb) {
    try {
        ...
        that.add = function(key, cb0) {
            var bits = calculateIndexes(key, spec.env.size, spec.env.slices);
